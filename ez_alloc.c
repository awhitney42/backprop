...
/* allocate a float vector with range v[nl..nh] */
{
  ...
  if (!v) nrerror ("allocation failure in vector2()");
  return v-nl+NR_END;
}
...
/* allocate a double vector with range v[nl..nh] */
{
  ...
  if (!v) nrerror ("allocation failure in dvector2()");
  return v-nl+NR_END;
}
...
/* allocate an integer vector with range v[nl..nh] */
{
  ...
  if (!v) nrerror ("allocation failure in ivector2()");
  return v-nl+NR_END;
}
...
/* allocate a long integer vector with range v[nl..nh] */
{
  ...
  if (!v) nrerror ("allocation failure in lvector2()");
  return v-nl+NR_END;
}
...
/* allocate an unsigned int vector with range v[nl..nh] */
{
  ...
  if (!v) nrerror ("allocation failure in uivector2()");
  return v-nl+NR_END;
}
...
/* allocates an unsigned char vector with range v[nl..nh] */
{
  ...
  if (!v) nrerror ("allocation failure in ucvector2()");
  return v-nl+NR_END;
}
...
/* allocates an char vector with range v[nl..nh] */
{
  ...
  if (!v) nrerror ("allocation failure in cvector2()");
  return v-nl+NR_END;
}
...
/* allocate a float 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
  ...
  if(!t) nrerror("allocation failure 1 in f3tensor2()");
  t += NR_END;
  t -= nrl;

  /* allocate pointers to rows and set pointers to them */
  t[nrl] = (float **) malloc((size_t)((nrow*ncol+NR_END)*sizeof(float*)));
  if(!t[nrl]) nrerror("allocation failure 2 in f3tensor2()");
  t[nrl] += NR_END;
  t[nrl] -= ncl;

  /* allocate rows and set pointers to them */
  t[nrl][ncl] = (float *) malloc((size_t)((nrow*ncol*ndep+NR_END)*sizeof(float)));
  if(!t[nrl][ncl]) nrerror("allocation failure 3 in f3tensor2()");
  t[nrl][ncl] += NR_END;
  t[nrl][ncl] -= ncl;
...
/* allocate a long 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
  ...
  if(!t) nrerror("allocation failure 1 in l3tensor2()");
  t += NR_END;
  t -= nrl;

  /* allocate pointers to rows and set pointers to them */
  t[nrl] = (long **) malloc((size_t)((nrow*ncol+NR_END)*sizeof(long*)));
  if(!t[nrl]) nrerror("allocation failure 2 in l3tensor2()");
  t[nrl] += NR_END;
  t[nrl] -= ncl;

  /* allocate rows and set pointers to them */
  t[nrl][ncl] = (long *) malloc((size_t)((nrow*ncol*ndep+NR_END)*sizeof(long)));
  if(!t[nrl][ncl]) nrerror("allocation failure 3 in l3tensor2()");
  t[nrl][ncl] += NR_END;
  t[nrl][ncl] -= ncl;
...
/* allocate a double 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
  ...
  if(!t) nrerror("allocation failure 1 in d3tensor2()");
  t += NR_END;
  t -= nrl;

  /* allocate pointers to rows and set pointers to them */
  t[nrl] = (double **) malloc((size_t)((nrow*ncol+NR_END)*sizeof(double*)));
  if(!t[nrl]) nrerror("allocation failure 2 in d3tensor2()");
  t[nrl] += NR_END;
  t[nrl] -= ncl;

  /* allocate rows and set pointers to them */
  t[nrl][ncl] = (double *) malloc((size_t)((nrow*ncol*ndep+NR_END)*sizeof(double)));
  if(!t[nrl][ncl]) nrerror("allocation failure 3 in d3tensor2()");
  t[nrl][ncl] += NR_END;
  t[nrl][ncl] -= ncl;
...
double ****d4dim2(double ****t,long d1l, long d1h, long d2l, long d2h, long d3l, long d3h, long d4l, long d4h)
{
  ...
  if(!t[d1l]) nrerror("allocation failure 2 in d4tensor()");
  t[d1l] += NR_END;
  t[d1l] -= d2l;
  
  t[d1l][d2l] = (double **) malloc((size_t)((gap1*gap2*gap3+NR_END)*sizeof(double*)));
  if(!t[d1l][d2l]) nrerror("allocation failure 3 in d4tensor()");
  t[d1l][d2l] += NR_END;
  t[d1l][d2l] -= d3l;
  
  t[d1l][d2l][d3l] = (double *) malloc((size_t)((gap1*gap2*gap3*gap4+NR_END)*sizeof(double)));
  if(!t[d1l][d2l][d3l]) nrerror("allocation failure 4 in d4tensor()");
  t[d1l][d2l][d3l] += NR_END;
  t[d1l][d2l][d3l] -= d4l;
